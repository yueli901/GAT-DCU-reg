...
class Encoder(tf.keras.Model):
    ...
    def call(self, feature, data):
        ...
        states = [] # hold the two output final state of the MyGRU layer and the MetaGRU layer
        for depth, cell in enumerate(self.cells):
            ...
            if self.graphs[depth] is not None:
                ...
                for g in self.graphs[depth]:
                    s, rho = g(data, feature)
                    s_inout.append(s)
                    rho_inout.append(rho)
                data = tf.reduce_mean(s_inout, axis=0)
                rho_matrix_encoder = tf.concat(rho_inout, axis=-1) # [n, b, t, 2]

        return states, rho_matrix_encoder
...
class Decoder(tf.keras.Model):
    ...
    def call(self, feature, label, begin_states, is_training):
        ...
        for i in range(seq_len):
            ...
            for depth, cell in enumerate(self.cells):
                ...
                if self.graphs[depth] is not None:
                    ...
                    for g in self.graphs[depth]:
                        ...
                        rho_inout.append(rho)
                    data = tf.reduce_mean(s_inout, axis=0)
                    rho_matrix_decoder_1t = tf.concat(rho_inout, axis=-1) # [n, b, t, 2]
            ...
            output.append(data) # add the newest prediction to the total output sequence
            rho_matrix_decoder.append(rho_matrix_decoder_1t)

        output = tf.stack(output, axis=2)
        rho_matrix_decoder = tf.stack(rho_matrix_decoder, axis=2)
        return output, rho_matrix_decoder
...
class Seq2Seq(tf.keras.Model):
    ...
    def call(self, feature, data, label, is_training):
        ...
        states, rho_matrix_encoder = self.encoder(feature, data)
        
        # Seq2Seq decoding process
        output, rho_matrix_decoder = self.decoder(feature, label, states, is_training)  # output [n, b, t, d], rho_matrix_decoder [n, b, t_out, 2 (in & out)]
        rho_matrix = tf.concat([rho_matrix_encoder, rho_matrix_decoder], axis=2) # [n, b, t_in + t_out, 2]
        
        return output, rho_matrix
