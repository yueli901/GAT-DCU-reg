...
def train_model(net, train_dataset, eval_dataset, node_features, scaler, num_epochs, optimizer, clip_gradient, lambda_reg):
    for epoch in range(num_epochs):
        for step, (data, label, _, _) in enumerate(train_dataset):
            with tf.GradientTape() as tape:
                ...
                pred, rho_matrix = net(node_features, data, label, is_training=True)
                # regularization term for dynamic rho matrix with size [n=498, b=32, t=12+1 (encoder & decoder), 2 (in & out)]
                rho_reg = tf.reduce_mean(tf.abs(rho_matrix))
                mse, mse_speed, mse_volume, mae_speed, mae_volume = compute_loss(label, pred, scaler)
                total_loss = mse + lambda_reg * rho_reg
...
def evaluate_model(net, dataset, node_features, scaler):
    ...
    for data, label, _, _ in dataset:
        data = tf.transpose(data, perm=[1, 0, 2, 3])  # [b, t, n, d] to [n, b, t, d]
        label = tf.transpose(label, perm=[1, 0, 2, 3])  # [b, t, n, d] to [n, b, t, d]
        pred, _ = net(node_features, data, label, is_training=False)
        mse, mse_speed, mse_volume, mae_speed, mae_volume = compute_loss(label, pred, scaler)
        MSE += mse
        MSE_speed += mse_speed
...
def main(args):
    ...
    current_time = datetime.datetime.now()
    formatted_time = current_time.strftime('%Y%m%d-%H%M%S')
    filename = f"{model_setting['name']}-{str(15*int(dataset_setting['time_interval']))}min-{str(args.epochs)}-lambda{train_setting['lambda_reg']}.h5"
    ...
    train_model(net, train_dataset, eval_dataset, node_features, scaler, args.epochs, optimizer, train_setting['clip_gradient'], lambda_reg=train_setting['lambda_reg'])
    ...
    os.rename(os.path.join(EVALUATE_PATH,'training_log.log'), os.path.join(EVALUATE_PATH,f"training_log-{str(15*int(dataset_setting['time_interval']))}min-{str(args.epochs)}-lambda{train_setting['lambda_reg']}.log"))
